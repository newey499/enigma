...
QSqlRecord ReflectorData::getEmptyReflector()
{
    rec.clear();
...
QSqlRecord ReflectorData::getReflector()
{
    return rec;
}
...
bool ReflectorData::setId(int id)
{
    bool result = true;
    clearError();
...
bool ReflectorData::setName(QString name)
{
    bool result = true;
    clearError();
...
bool ReflectorData::setAlphabetId(int id)
{
    bool result = false;
    clearError();
...
bool ReflectorData::setType(QString type)
{
    bool result = true;
    clearError();

    // Should always be "REFLECTOR"
...
bool ReflectorData::setPinRight(QString pinRight)
{
    bool result = true;
    clearError();
...
bool ReflectorData::setNotches(QString notches)
{
    bool result = true;
    clearError();
...
bool ReflectorData::validateReflector(Globals::EDIT_MODE mode)
{
    bool result = true;
    QString msg;
    QSqlQuery qry;
    ...
    case Globals::ROW_ADD :
        ...
        // Type must be "REFLECTOR"
        if (rec.value("type").toString().compare("REFLECTOR") != 0)
        {
            msg = QString("type must be \"REFLECTOR\" not [%1]").
        ...
        if (! isMinLengthOk(rec.value("pinright").toString(), "Reflector pinright"))
        {
            result = false;
        }
    ...
    case Globals::ROW_EDIT :
        ...
        if (GenLib::execQry(qry, true))
        {
            ...
            {
                msg = QString("[%1] occurences of id [%2] on reflector table").
        ...
        if (rec.value("alphabetid").toInt() == Globals::NEW_ID)
        {
            msg = QString("Reflector Id must not be a new record id [%1]").
                            arg(Globals::NEW_ID);
            addError(msg);
            result = false;
        }
    ...
    case Globals::ROW_DEL :
        ...
        {
            msg = QString("cannot delete - Reflector Wheel [%1] is in use").
                    arg(rec.value("id").toInt());
            addError(msg);
            result = false;
        }
        ...
        if (result)
        {
            msg = QString("Deletion validation for Reflector wheel row id [%1] passed").
                                arg(rec.value("id").toInt());
            addError(msg);
        }
        else
        {
            msg = QString("Deletion validation for Reflector wheel row id [%1] failed").
                                arg(rec.value("id").toInt());
            addError(msg);
        }
...
bool ReflectorData::writeRec(Globals::EDIT_MODE mode)
{
    bool result = true;
    QSqlQuery qry;
    QString msg;

    if (! validateReflector(mode))
    {
    ...
    switch (mode)
    {
        case Globals::ROW_ADD :
            ...
            if (result)
            {
                ...
                if (result)
                {
                    ...
                    msg = QString("rotor table row added - REFLECTOR wheel id = [%1]").
                            arg(rec.value("id").toInt());
                    addError(msg);
                    result = true;
                }
        ...
        case Globals::ROW_EDIT :
            ...
            {
                msg = QString("rotor table row updated - REFLECTOR wheel id = [%1]").
                        arg(rec.value("id").toInt());
                addError(msg);
                result = true;
            }
        ...
        case Globals::ROW_DEL :
            ...
            {
                msg = QString("Deleting Reflector Wheel id = [%1]").
                        arg(rec.value("id").toInt());
                addError(msg);
                result = true;
            }
